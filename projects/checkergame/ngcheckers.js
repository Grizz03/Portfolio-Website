...
angular.module('ngCheckers', [])
  ...
  .controller('checkersCtrl', function($scope, $timeout) { 
    ...
    $scope.setClass = function(square) {
      if (square.y % 2 === 0) {
        if (square.x % 2 === 0) {
          return {"backgroundColor": square.isChoice ? "green" : "rgb(139,69,19)"};
        } else {
          return {"backgroundColor": "rgb(205,133,63)"};
        }
      } else {
        if (square.x % 2 === 1) {
          return {"backgroundColor": square.isChoice ? "green" : "rgb(139,69,19)"};
...
function pieceMove () {
  var audio = new Audio('checkerMove.wav');
  audio.play();
}
function cheer () {
...
function ohyeah () {
  var audio = new Audio('ohyeah.wav');
  audio.play();
}